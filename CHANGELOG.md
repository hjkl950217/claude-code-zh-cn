...
- Windows native 支持窗口上限提升到 2.1.278：常量池翻译命中 1163 条，显示审计 11/11 通过。
- 补译 57 条 CLI 帮助文本（`claude --help`、`--cloud`、`--plugin`、`auto-mode` 等）。其中 23 条主表本就有完整译文，但长度超出 bytecode 窄槽会被静默跳过，界面因此长期显示英文；现由池内专用短译覆盖主表，其余 34 条为池路径新增。
- 支持窗口未收录的 2.1.266 ~ 2.1.277 明确列入 excluded，避免被误认为已验证。

## [2.17.0] - 2026-09-16

### 改进

- 原生 Claude Code 接入启动前汉化：通过安装器配置一次启动入口后，普通升级和同版本重装在下次 `claude` 启动时自动检测并修复，无需重新安装插件。
- 安装器与 Unix 会话 Hook 共用补丁事务；验证副本后才替换实际程序，通过文件指纹识别更新、备份和规则变更，保护运行中的文件与并发更新。
- Linux x64 glibc 未收录版本也能按程序结构进行本机验证；发布支持清单仍仅记录已验证版本，不增加 arm64 / musl 支持。
- 启动器从正式插件安装记录选择当前版本，市场更新后在下一次启动使用新规则，并尊重停用设置；启动前不请求网络。
- 三平台新版检查成功后自动生成适配 PR，并显式运行分支 CI；不自动合并或发布。

### 修复

- 移除以禁用 Claude Code 自动更新维持汉化的安装提示和开关。
- Windows 原生程序已被占用时明确提示关闭占用窗口后再次启动，不再要求重复安装插件。

## [2.16.1] - 2026-09-14

### 修复
- 增强安装同步 CC Switch 时，先读取并合并已有通用配置，保留插件启用开关、用户字段和嵌套配置；无效 JSON 拒绝写入。Node.js 内置 SQLite 和 sqlite3 两条路径均覆盖回归测试。
- 原生字节码补丁保护多行粘贴、图片等协议占位符，避免粘贴正文被当成普通标记文本。

### 改进
- 补全文案：展开提示、退出对话框、后台任务、目标状态和上下文压缩提示；保护被程序逻辑消费的通知前缀。
- 完整 shell 安装器在缺少 sqlite3 时明确提示 CC Switch 通用配置未同步，包括静默更新场景。

### 新增
- 内网离线安装指南：准备便携 Node.js、原生处理依赖和本地插件市场，说明禁止 Node.js、系统兼容性及离线模型服务的边界。
...
- 区分计划批准与拒绝提示，避免两种结果被翻译成相同的不完整句子。
- Windows 独占写入检查及安装、卸载共用的文件替换步骤等待短暂占用释放，修复启动自检后安装或卸载被文件锁中止；持续占用时仍停止写入并保留原程序。
...
- macOS arm64 与 Windows x64 native 支持窗口同步跟进 Claude Code `2.1.237`，插件版本推进到 `2.12.2`。

### 验证

- macOS 与 Windows 的 extract / patch（1603 处）/ repack / codeSignature / `--version` 全链路通过，11 个稳定显示面运行审计通过；新增 help 文案仍有 29 条未翻译警告，不影响运行兼容性，也不代表完整中文覆盖。
- `CI preflight`
...
- 收紧 native 候选验证：执行外部候选的任务改为只读且不保留 Git 凭据，`node-lief` 固定为 1.3.2；失败报告与支持证据仅上传产物，不再由候选任务直接写 Issue 或 PR。
- 远程 Release 安装把已校验提交的本地快照注册进插件市场，并阻止 SessionStart 回到可变仓库自动更新；发现新版本时只提示重跑固定 Release 安装器。
- 上游包缓存必须是当前用户私有目录；原生 JS 提取拒绝跟随符号链接，临时输出改用私有随机目录。
- GitHub 令牌通过标准输入传给 curl 请求头，不再出现在进程参数；设置文件原子替换保持仅当前用户可读写权限。

### 验证

- 新增共享缓存、符号链接、令牌参数、固定快照市场和设置权限回归测试；CI 权限契约同步覆盖 macOS 与 Windows native 候选流程。

## [2.10.1] - 2026-08-05

### 修复

- 修复插件市场安装与 SessionStart 将 `spinnerVerbs` 写成数组、导致 Claude Code 跳过整个 `settings.json` 的问题；现在始终写入包含 `mode` 和 `verbs` 的合法对象。
- 已被旧版本写入数组的设置会在下次 SessionStart 或增强安装时无损迁移为对象，保留用户原有动词；旧安装缓存与 Windows PowerShell 路径同步修复。

### 验证

- 新增对象输出、旧数组迁移、缓存修复和 CC Switch 识别回归测试；合法的用户自定义对象继续原样保留。

## [2.10.0] - 2026-08-02

### 改进

- 为 `/status` 和设置页的 7 个高频术语保留英文对照，方便按英文教程定位；`Session name` 与 `Session ID` 分别译为“会话名称（Session name）”和“会话 ID（Session ID）”，不再混淆两个字段。
- README 补充临时切回英文、重新启用和卸载入口，并优化长文档的导航与标题层级。

### 修复

- doctor 正确识别远程安装记录的 GitHub `owner/repo` 来源，不再把合法仓库名误报为不存在的本地路径；真实缺失的本地源码路径仍会告警。

### 验证

- Claude Code 2.1.220 Linux x64 原生包完成 1492 处 patch；7 个中英并列词条全部命中，ELF 回写后逐字读回一致。
- 翻译质量、翻译结构、插件 payload、CLI patch 与 doctor 相关回归测试通过。
...
- macOS arm64 与 Windows x64 native 不再用版本号做运行硬门禁：高于已知下限的可识别版本走本机 provisional 自检，已知文案继续中文，新文案保持英文。
...
- 正式插件校验失败转入独立兜底时，会先停用未确认的官方入口并留下可重试标记；下一次安装可恢复正式注册，但同一会话周期不会加载两套 Hook。
...
- npm `cli.js` 写回改用进程唯一临时文件和可回滚替换，SessionStart 对 native / npm 共用跨进程锁，避免并发 Hook 混写同一个 CLI 文件。
...
- 新增跨版本线 native 文案部分覆盖、失败回滚、正式注册/备用 Hook 互斥、精准卸载和版本清单一致性回归测试。

## [2.5.1] - 2026-07-09

### 改进

- macOS / Windows native latest 跟进 Claude Code `2.1.205`：双平台验证通过后同步支持窗口、README / support matrix 派生产物，并把插件版本推进到 `2.5.1`，合并后可按发布流程创建 `v2.5.1`。
...
- macOS native latest 自动 closeout 跟进 Claude Code `2.1.185`：验证通过后同步支持窗口、README / support matrix 派生产物，并把插件版本推进到 `2.4.61`，合并后可按发布流程创建 `v2.4.61`。
...
- macOS / Windows native latest 自动 closeout 跟进 Claude Code `2.1.183`：验证通过后同步支持窗口、README / support matrix 派生产物，并把插件版本推进到 `2.4.60`，合并后可按发布流程创建 `v2.4.60`。
...
- macOS / Windows native latest 自动 closeout 跟进 Claude Code `2.1.179`：验证通过后同步支持窗口、README / support matrix 派生产物，并把插件版本推进到 `2.4.59`，合并后可按发布流程创建 `v2.4.59`。
...
- macOS / Windows native latest 自动 closeout 跟进 Claude Code `2.1.178`：验证通过后同步支持窗口、README / support matrix 派生产物，并把插件版本推进到 `2.4.58`，合并后可按发布流程创建 `v2.4.58`。
...
- 补充 Claude Code `2.1.177` 的 `/effort` 思考强度选择界面和动态 workflow 界面翻译，覆盖选择器标题、速度/智能提示、workflow 空状态、运行确认、后台运行提示和冲突提示；同时补齐信任弹窗快捷键、主界面 Agent 入口和取消提示等公共可见残留。
