...
[![Windows native](https://img.shields.io/badge/windows%20native-2.1.113--2.1.278-green)](./docs/support-matrix.md)
...
- ✅ 已验证版本直接使用公开证据；更高 native 版本也先本机自检。可 patch 硬编码文字（2104 条翻译；代表版本 `2.1.112` 实测 1740 处有效 patch）
...
| `powershell -File install.ps1` | Windows：旧 npm cli.js 最完整；native .exe `2.1.113 - 2.1.278` 内已验证版本需 `node-lief`；Claude 更新后关闭所有窗口并重跑 |
...
| Windows · native .exe（x64） | `2.1.113 - 2.1.278` 内的已验证版本 | 需要 `node-lief`；个别版本未收录，见支持矩阵 |
...
| UI 文字中文化 | 2104 条翻译，`2.1.112` 实测 1740 处有效 patch | CLI Patch（扫描真实双引号字符串 token 后逐条替换）+ 显示面审计 |
