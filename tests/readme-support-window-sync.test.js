...
test("README support window sync rewrites badges, support table, and install advice from config", () => {
  ...
  assert.match(text, /\| Linux · native binary（x64 glibc） \| `2\.1\.220 - 2\.1\.220` \| 需要 `node-lief >=1\.3\.0`；未收录版本先本机验证，不含 arm64、musl \|/);
  ...
  assert.match(text, /\*\*Linux 本机验证\*\*/);
  assert.match(text, /Linux x64 glibc 的未收录版本先按程序结构和启动自检验证/);
